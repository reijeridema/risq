# Build model for fiting survey data.
build_model <- function(formula, family) {
  list(formula = formula, family = family)
}

# Get variables used in left-hand side of formula.
get_lhs_variables <- function(formula) {
  lhs <- if(length(formula) > 2) formula[[2L]] else NULL
  all.vars(lhs)
}

# Get variables used in right-hand side of formula.
get_rhs_variables <- function(formula) {
  rhs <- if(length(formula) > 2) formula[[3L]] else formula[[2L]]
  all.vars(rhs)
}
