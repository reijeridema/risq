...
test_that("bootstrap works with ri", {
  test_bootstrap_values(robj_1, ri, target = "response_1", seed = 0)
})

test_that("bootstrap works with ri_by_var", {
  variables <- c("age", "urbanisation")
  test_bootstrap_values(
    robj_1, ri_by_var, target = "response_1", variables = variables, seed = 1
  )
})
...
test_that("bootstrap works with ri_by_cat", {
  ...
  test_bootstrap_values(
    robj_1, ri_by_cat, target = "response_1", variables = variables, seed = 2
  )
})

test_that("bootstrap works with cv", {
  test_bootstrap_values(robj_1, cv, target = "response_1", seed = 42)
})

test_that("bootstrap works with cv_by_var", {
  variables <- c("age", "urbanisation")
  test_bootstrap_values(
    robj_1, cv_by_var, target = "response_1", variables = variables, seed = 43
  )
})
...
test_that("bootstrap works with cv_by_cat", {
  variables <- c("age", "marital_status")
  test_bootstrap_values(
    robj_1, cv_by_cat, target = "response_1", variables = variables, seed = 44
  )
})
...
test_that("bootstrap detects invalid input", {
  ...
  class(robj) <- "rix"
  expect_error(
    bootstrap(robj, ri, target = "response_1", iterations = 1),
  ...
  robj <- risq(~ age + gender, data = data_1)
  expect_error(
    bootstrap(robj, ri, target = "response_1", seed = "", iterations = 1),
    "`seed` must be an integer"
  )
  expect_error(
    bootstrap(robj, ri, target = "response_1", seed = c(0, 1), iterations = 1),
    "`seed` must be an integer"
  )
  ...
  robj <- risq(~ age + gender, data = data_1)
  expect_error(
    bootstrap(robj, ri, target = "response_1", iterations = 0),
  ...
  expect_error(
    bootstrap(robj, ri, target = "response_1", iterations = "ten"),
    "`iterations` must be a positive integer"
  )
})
