...
test_ri_vs_ref <- function(family) {
  ...
  expect_equal(which(is.na(ri_tst)), c(21, 22, 23, 26, 27, 28))
...
test_that("ri_by_cat detects invalid input", {
  ...
  class(robj) <- "risk"
  expect_error(ri_by_cat(robj, "r", "x"), "`robj` must be a risq object")

  # Test invalid target argument.
  robj <- risq(~ x, "binomial", data_1)
  expect_error(ri_by_cat(robj, factor("r"), "x"), "`target` must be a string")
  expect_error(ri_by_cat(robj, "x", "x"), "`target` variable must not be in risq predictor")
  expect_error(ri_by_cat(robj, "Q", "x"), "`target` variable must be present in risq data")
  expect_error(ri_by_cat(robj, "y", "x"), "`target` variable must be logical in risq data")

  # Test missing values in target.
  data_missing_r <- data_1
  data_missing_r$r[9] <- NA
  robj <- risq(~ x + y, "gaussian", data_missing_r)
  expect_error(ri_by_cat(robj, "r"), "`target` variable must not contain `NA` values")

  # Test invalid variables argument.
  robj <- risq(~ x + y, "binomial", data_1)
  expect_error(ri_by_cat(robj, "r", 0), "`variables` must be a character vector")
  expect_error(ri_by_cat(robj, "r", c("x", "r")), "`variables` must not contain the target")
  expect_error(ri_by_cat(robj, "r", c("x", "Q")), "`variables` must be present in risq data")
  expect_error(ri_by_cat(robj, "r", c("x", "y")), "`variables` must be factors in risq data")
  expect_error(ri_by_cat(robj, "r", c("s", "x")), "`variables` must be factors in risq data")
  ...
  expect_error(ri_by_cat(robj, "r", c("z", "x")), "`variables` must not contain `NA` values")
})
...
test_that("ri_by_cat handles variables with empty levels", {
  ...
  result_1 <- ri_by_cat(robj_1, "r", c("x", "z"))
  robj_2 <- risq(~ x + y, "binomial", data_2)
  result_2 <- ri_by_cat(robj_2, "r", c("x", "z"))
