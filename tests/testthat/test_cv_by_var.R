...
test_cv_vs_ref <- function(family) {
  ...
  cv_tst_u <- cv_by_var(robj, target, variables, "unconditional")
  expect_equal(cv_tst_u, cv_ref$unconditional)
  cv_tst_u_no_se <- cv_by_var(robj, target, variables, "unconditional", FALSE)
  expect_equal(cv_tst_u_no_se, cv_tst_u[-3])
  cv_tst_c <- cv_by_var(robj, target, variables, "conditional")
  expect_equal(cv_tst_c, cv_ref$conditional)
  cv_tst_c_no_se <- cv_by_var(robj, target, variables, "conditional", FALSE)
  expect_equal(cv_tst_c_no_se, cv_tst_c[-3])
  ...
  expect_equal(which(is.na(cv_tst_c)), c(4, 6))
...
test_that("cv_by_var detects invalid input", {
  ...
  class(robj) <- "risque"
  expect_error(cv_by_var(robj, "r", "x"), "`x` must be a risq object")

  # Test invalid target argument.
  robj <- risq(~ x, "gaussian", data_1)
  expect_error(cv_by_var(robj, c("r", "s"), "x"), "`target` must be a string")
  expect_error(cv_by_var(robj, "x", "x"), "`target` must not be in risq predictor")
  expect_error(cv_by_var(robj, "X", "x"), "`target` must be present in risq data")
  expect_error(cv_by_var(robj, "y", "x"), "`target` must be of type logical in risq data")

  # Test missing values in target.
  data_missing_r <- data_1
  data_missing_r$r[9] <- NA
  robj <- risq(~ x + y, "binomial", data_missing_r)
  expect_error(cv_by_var(robj, "r"), "`target` must not contain `NA` values in risq data")

  # Test invalid variables argument.
  robj <- risq(~ x + y, "gaussian", data_1)
  expect_error(cv_by_var(robj, "r", 0), "`variables` must be a character vector")
  expect_error(cv_by_var(robj, "r", c("x", "r")), "`variables` must not contain target variable")
  expect_error(cv_by_var(robj, "r", c("x", "q")), "`variables` must be present in risq data")
  expect_error(cv_by_var(robj, "r", c("x", "y")), "`variables` must be of type factor in risq data")
  expect_error(cv_by_var(robj, "r", c("s", "x")), "`variables` must be of type factor in risq data")
  ...
  expect_error(cv_by_var(robj, "r", c("x", "z")), "`variables` must not contain `NA` values in risq data")

  # Test invalid type argument.
  robj <- risq(~ x + y, "gaussian", data_1)
  expect_error(cv_by_var(robj, "r", "x", type = TRUE), "character vector")

  # Test invalid include_se argument.
  robj <- risq(~ x + y, "gaussian", data_1)
  expect_error(cv_by_var(robj, "r", "x", include_se = "TRUE"), "`include_se` must be TRUE or FALSE")
})
